...
@blueprint.route("/<team_id>/checks/services", methods=['GET'])
@login_required
@requires_roles('administrator', 'organizer')
@requires_no_parameters
def get_all_service_checks_for_team(team_id):
    data = g.db.get_all_completed_service_checks_for_team(team_id)
    convert_all_datetime_to_timestamp(data, ['timestamp', 'time_to_check'])
    js = json.dumps(data, default=json_util.default)
    resp = Response(js, status=200, mimetype='application/json')
    return resp

@blueprint.route("/<team_id>/checks/services/<check_id>", methods=['GET'])
@login_required
@requires_roles('administrator', 'organizer')
@requires_no_parameters
