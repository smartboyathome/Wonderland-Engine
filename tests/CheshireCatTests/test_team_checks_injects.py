...
class TestRestTeamChecksInjectsInterface(FlaskTestCase):
    def test_get_all_inject_checks_for_specific_team(self):
        self.login_user('admin', 'admin')
        rest_result = self.app.get('/teams/1/checks/injects')
        print rest_result.status_code, rest_result.data
        assert rest_result.status_code == 200
        expected_result = [obj for obj in self.data['completed_checks'] if obj['team_id'] == '1' and obj['type'] == 'inject']
    ...
    def test_get_all_inject_checks_for_specific_team_with_params(self):
        self.login_user('admin', 'admin')
        query_data = {
            "failure": "assured"
        }
        ...
        result = self.app.get('/teams/1/checks/injects', data=json.dumps(query_data))
        print result.data
    ...
    def test_get_specific_inject_check_for_specific_team_with_params(self):
        self.login_user('admin', 'admin')
        query_data = {
            "failure": "assured"
        }
